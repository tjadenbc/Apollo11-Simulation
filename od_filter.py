"""OD FILTER (rung a) — pure DSN information-matrix covariance primitives.

Self-contained (numpy only; no apollo11 import -> no circular dependency). The apollo11 wiring
...
NATURE-CLASS PORT from the Artemis I sibling (byte-identical primitives; geometry-derived Fisher
information is period-neutral estimation math). The apollo11 caller uses only range + 2-way Doppler
(H_ddor is present for the shared self-test but NEVER invoked by apollo11: Delta-DOR is a 1970s+
VLBI technique that postdates Apollo and would violate the artifact guardrail).
...
def station_eci(lat_deg, lon_deg, alt_m, t, R_EARTH, OMEGA_E, GMST0):
    """DSN station geodetic (spherical Earth, matching the integrator's latlon->eci) -> ECI (r, v).
...
def _grav_gradient(gravity_fn, r, t, h=1.0):
    """G = d g / d r (3x3) by central finite difference of the true gravity model."""
    G = np.zeros((3, 3))
    for k in range(3):
        e = np.zeros(3); e[k] = h
...
def accumulate_covariance_stm(gravity_fn, t_epoch, s6_epoch, arc_len_s, n_samples, station_fn,
    ...
    """STM-based LinCov covariance at t_epoch. Propagate the state + state-transition matrix Phi(t,t_epoch)
    BACKWARD over the arc [t_epoch-arc, t_epoch]; a measurement at t_i informs the EPOCH state through
    the dynamics: info += (H_i Phi_i)^T R^-1 (H_i Phi_i). This is what pins VELOCITY (range/Doppler over an
    arc + the dynamics coupling), which the Phi~=I batch cannot. Adds process noise (q_*, grows the older
    measurements' effective error) and a systematic-error FLOOR (delivered OD is systematic-limited).
    Returns P (6x6, ECI). gravity_fn(r, t) -> accel; station_fn(t, r_v) -> list of (r_s, v_s)."""
...
def _fd_test():
    ...
    for k in range(6):
        ...
        Hn[k] = (rho_dot((x + dx)[:3], (x + dx)[3:]) - rho_dot((x - dx)[:3], (x - dx)[3:])) / (2 * eps[k])
