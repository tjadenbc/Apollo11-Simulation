...
crew and returning them safely below 5%, the program's design goals were 90% mission success
and 99.9% per-mission crew safety, commander Neil Armstrong privately reckoned a 90% chance of
returning alive but only even odds on the landing itself, and command module pilot Michael
Collins, in *Carrying the Fire*, would give "no better than even odds on a successful landing
and return." None of these rested on an end-to-end physical simulation of the flight. We present
...
lunar ephemeris and a degree-8 GRAIL gravity field, with Apollo's actual guidance laws
(iterative-guidance ascent, B-plane midcourse corrections, a predictor-corrector entry),
...
docking, surface operations, and entry overstress). Across 10,000 trials distributed over a
272-core cluster, we estimate **85.5% mission success (95% CI 84.7–86.1%)** and **93.4%
...
budgets, and 8.2-day duration to within a few percent. The estimate falls squarely between the
crew's own intuition and the program's eventual record of six successful landings in seven
...
On 16 July 1969, three men rode the largest rocket ever flown toward a body no human had
...
the one that could not be answered in 1969 except by judgment: *what was Apollo 11's actual
...
assessment for the remainder of the programme rather than publish it [1]. At the
optimistic extreme stood the programme's formal design goals: a mission-success probability of
0.90 and a crew-safety probability of 0.999 per flight — targets, it should be stressed, rather
than measured expectations. Between these poles lay the judgment of the people closest to the
...
the landing on the first attempt [2]. Command module pilot Michael Collins, in his memoir *Carrying
the Fire*, was blunter still: he "wouldn't give better than even odds on a successful landing and
return." [3] The wider world hedged in its own currencies — bookmakers who had offered a thousand to
one against a man reaching the Moon before 1971 closed their books entirely by launch week, and
the crew, unable to obtain ordinary life insurance, signed sheets of autographed covers for their
families to sell in the event they did not return [4].
...
fatalities across the eleven crewed Apollo missions, with Apollo 13 the program's single
...
Across ten thousand such trials, distributed over a 272-core computing cluster, we estimate a
...
all three returned to Earth alive — Kennedy's actual objective — even where a recovered in-flight
anomaly occurred. The remainder of the paper describes the simulation (Section 2), the
...
what it does and does not establish. The headline finding can be stated simply here: a faithful
...
Between thrusting events the spacecraft is propagated under the equation of motion

  **r̈** = −μ⊕ **r**/|**r**|³ + **a**_J2 + **a**☾ + **a**☉

where the first term is Earth point-mass gravity (μ⊕ = 3.986 004 418 × 10¹⁴ m³ s⁻²), **a**_J2
is the acceleration from Earth's oblateness (J₂ = 1.082 6267 × 10⁻³, R⊕ = 6 378 137 m), **a**☾
is third-body lunar gravity, and **a**☉ is third-body solar gravity. During powered flight an
additional thrust term **a**_thrust = (T/m) **û** is added along the guidance-commanded
direction, with the mass depleted according to ṁ = −T/(I_sp g₀) (g₀ = 9.806 65 m s⁻²) so that
propellant consumption is tracked as the seventh state rather than assumed.
...
from a real ephemeris rather than an idealisation. With the production configuration the Moon's
position is evaluated from a truncated Meeus series [5] (Astronomical Algorithms, ch. 47) at the
...
return — the idealised circular Moon (fixed inclination, node on the +X axis) is retained only as
a flag-disabled fallback for reproducing legacy results.

Lunar gravity itself is modelled to degree and order eight using the GRAIL **GRGM1200A** [6]
spherical-harmonic field (coefficients of degrees 2–12 embedded directly from the NASA Planetary
Data System, with the implied J₂ and C₂₂ checked against the code's independently sourced lunar
constants to within 0.003 %). The field is evaluated with a singularity-free Cunningham
...
Numbers* [7] and Smithsonian sources, with the service-module dry mass re-derived from the CSM-107
...
rated performance: the Service Propulsion System at 91 200 N and a specific impulse of 314.5 s;
the descent engine throttleable between 45 040 N and 4 660 N (its 10 % minimum); and the ascent
engine at 15 700 N. Because every burn is integrated rather than applied as an instantaneous
...
recovery zone* — the standard RTCC practice — placed 2 784 km along that trial's own entry
...
mission duration of about 8.18 days against Apollo 11's 8.16.
...
generated up front in the main process from a single seeded random stream and dispatched to
workers by trial index, so the serial and parallel drivers produce *bit-identical* results;
new dispersion draws are always appended at the end of the stream so that adding a failure mode
does not perturb the trials of earlier runs. Results are checkpointed after every trial, making
runs gap-safe and resumable. The definitive run reported here used seed 37 and 10 000 trials.

### 2.6 Implementation and compute

The simulator is implemented in Python (NumPy/SciPy) in roughly 6 800 lines. A single trial
costs about 350 s on a current laptop core and about 625 s on the older cluster cores used for
production. The definitive 10 000-trial run was executed on a 272-core SLURM cluster (17 nodes
of 16 cores), with the trial set partitioned into disjoint strided index subsets, run
independently, and merged. Because different machines carry different SciPy/BLAS builds, a
cluster run is not bit-identical to the laptop and is treated as its own population; the
laptop-derived targeting products (the entry-interface and B-plane aim points and the launch/TLI
preset) are pinned into the run so that cross-machine numerical skew cannot flip the marginal
nominal-trajectory branches. The complete source, the regression tests, and the definitive run
...
Two modelling rules apply throughout. First, the per-trial draws are taken in a fixed order
and appended to the end of the random stream, so that adding a mode never disturbs the trials
of an earlier run (Section 2.5). Second, the modes are evaluated in strict causal order along
...
| LM surface electrical failure | 0.85% (0.17 × 0.05) | An anomaly rate × an unrecoverable fraction; anchored to Apollo 11's snapped ascent-engine arming circuit-breaker. |
...
| 1201/1202 computer alarm | 15% occurrence, 97% recovery | Apollo 11 itself fired five guidance-computer alarms, all recovered — a recoverable design/procedural event (a radar-switch configuration), not a random hardware hazard. |
...
| High-g entry structural failure | *emergent* | Trajectory-dependent: a loss when the integrated peak deceleration exceeds the CM's ~12 g structural limit (NASA TN D-6725 [8] cites 10 g guidance / ~12 g structural). |
...
NASA Lunar Landing Operational Risk Model (LLORM) [9], the Launch Escape System's ~98% design
...
compares the nominal against the flown values (from the Saturn V Flight Evaluation Report [10] and
Orloff's *Apollo by the Numbers* [7]).
...
| APS propellant remaining (kg) | 380.0 | 380 | +0.0% |
| Peak entry deceleration (g) | 8.6 | 6.5 | +31.8% |
| Total mission duration (days) | 8.18 | 8.16 | +0.2% |
...
mission duration matches Apollo's 8.16 days to within a few hours, with the flown 26.7-hour
lunar-orbit-insertion-to-powered-descent interval held by construction.

Three rows diverge, and we report them rather than tune them away. The peak entry
...
HUNTEST-style reference-deceleration profile with closed-loop drag tracking, which a
six-variant development corpus did not bring to readiness; the guided entry instead flies a
...
accuracy. The lunar-approach periapsis sits ~12% low and the Lunar Module ascent altitude
runs high; both are second-order to the mission-success accounting and reflect the
simplified rendezvous and approach geometry noted among the limitations. The nominal
...
We ran 10,000 trials of the definitive configuration (seed 37) on a 272-core cluster.
A trial counts as a **mission success** if the crew reached the lunar surface *and* all
three astronauts returned to Earth alive — NASA's actual objective — even where a
recovered in-flight anomaly occurred; it counts as a **failure** only if the landing was
...
launch-and-ascent phases and on the translunar coast, consistent with the failure
decomposition below.
...
— accounts for about 6% of all trials; the Saturn V launch family is next at 4.3%. Notably,
translunar-injection propellant exhaustion contributes *zero* failures: the 4.8–5.9% "TLI
starve" of earlier development runs was traced to a wrap-boundary bug in the
ignition-crossing detector and eliminated.

![Failure-mode breakdown across the 1,455 failed missions](outputs/apollo11_final10000/failure_modes.png)

**Figure 3. Failure-mode breakdown.** The 1,455 mission failures, coloured by the mission
...
| Cause | Deaths | % of deaths | Trials | Lethality |
|---|---:|---:|---:|---|
| SM systems failure — trans-Earth | 477 | 26.8 | 159 | always 3 |
| High-g entry structural failure | 303 | 17.0 | 101 | always 3 |
| Launch structural failure (max-Q) | 270 | 15.2 | 90 | always 3 |
| LM surface electrical failure | 192 | 10.8 | 96 | always 2 |
| SM systems failure — surface | 108 | 6.1 | 36 | always 3 |
| LM tip-over at touchdown | 88 | 4.9 | 44 | always 2 |
| Parking-orbit decay | 81 | 4.6 | 27 | always 3 |
| Descent-propellant exhaustion | 74 | 4.2 | 37 | always 2 |
| SM systems failure — lunar orbit | 72 | 4.0 | 24 | always 3 |
| SM systems failure — translunar | 63 | 3.5 | 21 | always 3 |
| Crew-return-leg loss | 14 | 0.8 | 6 | variable (1–3) |
| Ascent-rendezvous docking failure | 10 | 0.6 | 5 | always 2 |
| S-IC underperformance | 9 | 0.5 | 3 | always 3 |
| EVA suit / PLSS failure | 8 | 0.4 | 8 | always 1 |
| Transposition & docking failure | 3 | 0.2 | 1 | always 3 |
| Missed lunar approach after MCC | 3 | 0.2 | 1 | always 3 |
| S-IVB first-ignition failure | 3 | 0.2 | 1 | always 3 |
| Landing-radar dropout | 2 | 0.1 | 1 | always 2 |
| **Total** | **1,780** | **100** | | |
...
guidance, aborts, and propellant margins actually do their work. It is, second, below the
programme's 90% mission-success *design goal* — but only modestly, and the goal was a target
rather than a prediction; the crew-survival estimate of 93.4%, likewise, sits below the
0.999 per-flight safety target while remaining well above the ~90% chance of returning alive
that Armstrong privately reckoned. And it is, third and most pointedly, on the optimistic
...
roughly even (≈0.93¹¹ ≈ 0.5) — which is to say the model regards Apollo's actual outcome, a
...
gap is the natural next fidelity frontier. The second caveat is sensitivity: the
...
targeting products, and the generated dashboard). Intermediate development runs are omitted to
...
2. Armstrong, N. A. Pre-flight assessment of Apollo 11's odds — roughly a 90% chance of
   returning alive but only even odds on a successful landing on the first attempt — recounted
   in a 2011 interview; reported in "For Neil Armstrong, the First Moon Walker, It Was All
   about Landing the Eagle," *Scientific American* (2012).
3. Collins, M. (1974). *Carrying the Fire: An Astronaut's Journeys.* Farrar, Straus and Giroux,
   New York.
4. Apollo insurance covers. The Apollo 11 crew, unable to obtain affordable in-flight life
...
6. Goossens, S., et al. (2016). *A Global Degree and Order 1200 Model of the Lunar Gravity
   Field Using GRAIL Mission Data (GRGM1200A).* 47th Lunar and Planetary Science Conference,
   Abstract #1484.
7. Orloff, R. W. (2000). *Apollo by the Numbers: A Statistical Reference.* NASA SP-2000-4029,
   NASA History Division, Washington, DC.
8. Graves, C. A., and Harpold, J. C. (1972). *Apollo Experience Report — Mission Planning for
   Apollo Entry.* NASA Technical Note TN D-6725, Manned Spacecraft Center, Houston, TX.
9. Mattenberger, C., Putney, B., Rust, R., and Derkowski, B. (2010). *Lunar Landing
...
10. Marshall Space Flight Center (1969). *Saturn V Launch Vehicle Flight Evaluation Report —
